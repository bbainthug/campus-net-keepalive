...
- **账号有设备数上限（多设备互踢）** — 脚本本身无法突破上限。被别的设备挤下线时脚本会重登，反过来又把那台挤掉。为避免无限互踢，默认 10 分钟内重登超过 3 次就暂停 15 分钟（`MAX_RELOGINS` / `WINDOW` / `COOLDOWN`）。根治办法是减少设备数，比如用一台随身路由器认证，其他设备都接路由器，网关只看到一台设备。
